...
int saveGameToFile(GameBoard *gameBoard, Player currentPlayerTurn, int gameMode,
    ...
    if (writeTurn(currentPlayerTurn, fp) == FAIL) return FAIL;
    if (writeMode(gameMode, fp) == FAIL) return FAIL;
    if (writeColor(color, fp) == FAIL) return FAIL;
    if (writeDifficulty(difficulty, fp) == FAIL) return FAIL;
    if (writeGameBoard(gameBoard, fp, color) == FAIL) return FAIL;
...
int writeWholeTag(char *tagContent, char *tagName, FILE *fp) {
    if (writeOneTag(tagName, fp, 0) == FAIL
        || writeString(tagContent, fp) == FAIL
        || writeOneTag(tagName, fp, 1) == FAIL)
        return FAIL;
    else
        return SUCCESS;
}
...
int writeGameBoard(GameBoard *gameBoard, FILE *fp, Color color) {
    if (writeOneTag(BOARD, fp, 0) == FAIL) return FAIL;
    if (writeString("\n", fp) == FAIL) return FAIL;
    ...
    for (int r = LAST_ROW_CHAR; r >= FIRST_ROW_CHAR; r--) {
        ...
        if (writeWholeTag(row, rowTag, fp) == FAIL) return FAIL;
    }

    if (writeOneTag(BOARD, fp, 1) == FAIL) return FAIL;
    return SUCCESS;
}
...
int loadGameFromFile(GameBoard *gameboard, Player *currentPlayerTurn, int *gameMode,
    ...
    do{
        ...
        setPointer(tagName, tagContent, gameboard, currentPlayerTurn, gameMode, difficulty, color);
...
//        printf("the row is: %c\n\n", row);
        for(char col=FIRST_COL_CHAR; col<= LAST_COL_CHAR; col++)
        {
            ...
            if(pieceChar != '_');
            {
                int pieceIndex = getPieceIndexFromPieceChar(gameBoard, pieceChar, *color);
