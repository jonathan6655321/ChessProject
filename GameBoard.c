	...
	for (row = '1'; row <= '8'; row++) {
		for (col = 'A'; col <= 'H'; col++) {
			if (row == '1' || row == '2' || row == '7' || row == '8') {
				...
				gameBoard->mapLocationOnBoardToPieceIndex[rowColToLocationIndex(
						row, col)] = pieceIndex++;
			} else {
				gameBoard->mapLocationOnBoardToPieceIndex[rowColToLocationIndex(
						row, col)] = NO_PIECE;
...
void swapQueenKing(GameBoard *gameBoard) {
	...
	gameBoard->mapLocationOnBoardToPieceIndex[rowColToLocationIndex('1', 'E')]
			= PLAYER_1_QUEEN_INDEX;
	gameBoard->mapLocationOnBoardToPieceIndex[rowColToLocationIndex('1', 'D')]
			= PLAYER_1_KING_INDEX;
	gameBoard->mapLocationOnBoardToPieceIndex[rowColToLocationIndex('8', 'E')]
			= PLAYER_2_QUEEN_INDEX;
	gameBoard->mapLocationOnBoardToPieceIndex[rowColToLocationIndex('8', 'D')]
			= PLAYER_2_KING_INDEX;
}
 ...
 */
int movePiece(char rowFrom, char colFrom, char rowTo, char colTo,
		GameBoard *gameBoard) {
	if (isValidRowCol(rowFrom, colFrom) == FAIL || isValidRowCol(rowFrom,
			colFrom) == FAIL)
		return FAIL;
...
void setPieceAt(char row, char col, GameBoard *gameBoard, int pieceIndex) {
	...
	gameBoard->mapLocationOnBoardToPieceIndex[rowColToLocationIndex(row, col)]
			= pieceIndex;
	gameBoard->mapPieceIndexToLocationOnBoard[pieceIndex]
			= rowColToLocationIndex(row, col);
}
...
void removePieceAt(char row, char col, GameBoard *gameBoard) {
	...
	gameBoard->mapPieceIndexToLocationOnBoard[gameBoard->mapLocationOnBoardToPieceIndex[rowColToLocationIndex(
			row, col)]] = NOT_IN_GAME;
	gameBoard->mapLocationOnBoardToPieceIndex[rowColToLocationIndex(row, col)]
			= NO_PIECE;
}
...
int getIndexOfPieceAt(char row, char col, GameBoard *gameBoard) {
	...
	return gameBoard->mapLocationOnBoardToPieceIndex[rowColToLocationIndex(row,
			col)];
}
...
int getFirstPieceIndexForPlayer(Player player) {
	return (player == Player1) ? FIRST_PLAYER_1_PIECE_INDEX
			: FIRST_PLAYER_2_PIECE_INDEX;
}
int getLastPieceIndexForPlayer(Player player) {
	return (player == Player1) ? LAST_PLAYER_1_PIECE_INDEX
...
int numKnights(GameBoard *gameBoard, Player player) {
	int count = 0;
	if (player == Player1) {
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_1_KNIGHT_1]
				!= NOT_IN_GAME)
			count++;
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_1_KNIGHT_2]
				!= NOT_IN_GAME)
			count++;
		return count;
	} else {
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_2_KNIGHT_1]
				!= NOT_IN_GAME)
			count++;
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_2_KNIGHT_2]
				!= NOT_IN_GAME)
			count++;
		return count;
	}
...
int numBishops(GameBoard *gameBoard, Player player) {
	int count = 0;
	if (player == Player1) {
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_1_BISHOP_1]
				!= NOT_IN_GAME)
			count++;
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_1_BISHOP_2]
				!= NOT_IN_GAME)
			count++;
		return count;
	} else {
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_2_BISHOP_1]
				!= NOT_IN_GAME)
			count++;
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_2_BISHOP_2]
				!= NOT_IN_GAME)
			count++;
		return count;
	}
...
int numRooks(GameBoard *gameBoard, Player player) {
	int count = 0;
	if (player == Player1) {
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_1_ROOK_1]
				!= NOT_IN_GAME)
			count++;
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_1_ROOK_2]
				!= NOT_IN_GAME)
			count++;
		return count;
	} else {
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_2_ROOK_1]
				!= NOT_IN_GAME)
			count++;
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_2_ROOK_2]
				!= NOT_IN_GAME)
			count++;
		return count;
	}
...
int numQueens(GameBoard *gameBoard, Player player) {
	int count = 0;
	if (player == Player1) {
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_1_QUEEN_INDEX]
				!= NOT_IN_GAME)
			count++;
		return count;
	} else {
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_2_QUEEN_INDEX]
				!= NOT_IN_GAME)
			count++;
		return count;
	}
...
int numKings(GameBoard *gameBoard, Player player) {
	int count = 0;
	if (player == Player1) {
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_1_KING_INDEX]
				!= NOT_IN_GAME)
			count++;
		return count;
	} else {
		if (gameBoard->mapPieceIndexToLocationOnBoard[PLAYER_2_KING_INDEX]
				!= NOT_IN_GAME)
			count++;
		return count;
	}
 ...
 */
int getPieceIndexFromPieceChar(GameBoard *gameBoard, char pieceChar,
		Color player1Color) {
	Piece piece = { 0 };
