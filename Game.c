...
HandleCommandMessage handleSetDifficulty(Command command, Game *game) {
    ...
    if (game->gameMode == PlayerVsComputer) {
...
HandleCommandMessage handleSetMove(Command command, Game *game) {
    ...
    ResponseType response = executeUserMoveCommand(rowFrom, colFrom, rowTo, colTo, &(game->board), game->currentPlayer);
    switch (response) {
...
Command getComputerMove(Game *game) {
    ...
    if (game->difficulty == '5') {
        ...
        minimax(&(game->board), game->difficulty - '0', INT_MIN, INT_MAX,
                game->currentPlayer, scoreFunction, &miniMaxMove, 1);
    }
...
CheckmateType getCheckmate(Game *game) {
    ...
    if (hasMoves && (!thretened))
        return noCheck;
    if ((!thretened) && (!hasMoves))
        return tie;

    int check = thretened && hasMoves;
    int checkMate = thretened && (!hasMoves);
